...
// InitLogger 日志初始化
func InitLogger(filename string, maxSize, maxBackup, maxAge int, compress bool, longType string, level string) {
	// 获取日志写入介质
	writeSyncer := getLogWriter(filename, maxSize, maxBackup, maxAge, compress, longType)

	// 设置日志级别，具体请见 config/log.go
	logLevel := new(zapcore.Level)
	if err := logLevel.UnmarshalText([]byte(level)); err != nil {
		fmt.Println("日志初始化错误，日志级别设置有误。请修改config/log.go 文件中的 log.level 配置项")
	}
	...
	Logger = zap.New(
		...
		zap.AddStacktrace(zap.ErrorLevel), // Error 时才会现实 stactrac
	)

	// 将自定义的 Logger 替换为全局的logger
	// zap.L().Fatal() 调用时，就会使我们自定义的 Logger
	zap.ReplaceGlobals(Logger)

}

// 日志记录介质 os.Stdout 和文件
func getLogWriter(filename string, maxSize, maxBackup, maxAge int, compress bool, longType string) zapcore.WriteSyncer {

	// 如果配置了按照日期记录日志文件
	if longType == "daily" {
		longname := time.Now().Format("2006-01-02.log")
		filename = strings.ReplaceAll(filename, "logs.log", longname)
	}
